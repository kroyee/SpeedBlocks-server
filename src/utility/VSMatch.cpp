...
void VSMatch::addToQueue(Client& client, const sf::Time& _time) {
    ...
    client.sendPacket(PM::make<NP_ClientJoinedMatchmaking>());
    for (auto&& inqueue : queue)
        if (inqueue.client->id == client.id) return;
    for (auto&& inroom : playing)
        if (inroom.client->id == client.id) return;
...
void VSMatch::removeFromQueue(Client& client) {
    client.sendPacket(PM::make<NP_ClientLeftMatchmaking>());
...
bool VSMatch::checkQueue(const sf::Time& _time) {
    ...
    for (auto p1 = queue.begin(); p1 != queue.end(); p1++)
        for (auto p2 = p1; p2 != queue.end(); p2++) {
            ...
            if (p1->lastOpponent == p2->client->id) match += 200;
            if (p2->lastOpponent == p1->client->id) match += 200;
