...
void Node::decideGame() {
    ...
    if (nextgame == nullptr) {
        ...
        if (tournament.grade != 5) tournament.thread = new std::thread(&Tournament::scoreTournament, &tournament);
        return;
    }
...
void Node::sendResults(bool asPart) {
    ...
    for (unsigned int i = 0; i < result.p1_rounds.size(); i++) tournament.packet << result.p1_rounds[i];

    tournament.packet << (uint8_t)result.p2_rounds.size();
    for (unsigned int i = 0; i < result.p2_rounds.size(); i++) tournament.packet << result.p2_rounds[i];

    if (!asPart) tournament.sendToTournamentObservers(); */
}

void Node::sendScore() {
    if (room) {
        NP_GameScore gs;
        gs.scores.push_back({player1->id, result.p1_sets, 0, status});
        gs.scores.push_back({player2->id, result.p2_sets, 0, status});
        if (result.p1_rounds.size() > activeSet) gs.scores[0].rounds = result.p1_rounds[activeSet];
        if (result.p2_rounds.size() > activeSet) gs.scores[1].rounds = result.p2_rounds[activeSet];
...
void Node::sendReadyAlert() {
    ...
    PM packet;
    packet.write_as<NP_TournamentGameReady>(tournament.id, id);
    for (auto&& client : tournament.conn.clients) {
        if (client->id == player1->id || client->id == player2->id) client->sendPacket(packet);
    }
}
...
void Node::sendWaitTime(uint16_t waitTime, uint8_t player) {
    ...
    if (player == 1) {
        if (sendTime < player1->sentWaitingTime) {
            PM packet;
            packet.write_as<NP_TournamentTimeWo>(sendTime);
            room->clients.front()->sendPacket(packet);
...
void Node::resetWaitTimeSent() {
    if (player1 != nullptr) player1->sentWaitingTime = 6;
    if (player2 != nullptr) player2->sentWaitingTime = 6;
}
...
void Bracket::sendAllReadyAlerts() {
    for (auto&& game : games)
        if (game.status == 2) game.sendReadyAlert();
}

Tournament::Tournament(Connections& _conn)
    : conn(_conn), players(0), startingTime(0), status(0), scoreSent(false), scoreSentFailed(false), updated(false), notify(false) {}

bool Tournament::addPlayer(HumanClient& client) {
    for (auto&& player : participants)
        if (player.id == client.id) return false;
    Participant par;
    par.id = client.id;
...
void Tournament::addObserver(HumanClient& client) {
    for (auto&& observer : keepUpdated)
        if (observer->id == client.id) return;
...
void Tournament::setGameStatus() {
    for (auto&& game : bracket.games) {
        if (game.p1game == nullptr || game.p2game == nullptr)
            ...
            game.status = 1;
        if (game.player1 != nullptr && game.player2 != nullptr) game.status = 2;
    }
}
        ...
        if (game.depth == bracket.depth) {
            if (game.player1 != nullptr && game.player2 == nullptr) {
                if (game.nextgame->p1game->id == game.id)
                    ...
                    if (game.nextgame->player1 != nullptr && game.nextgame->player2 != nullptr) game.nextgame->status = 2;
                }
            }
        }
}
...
void Tournament::sendTournament() {
    PM packet;
    packet.write_as<NP_TournamentInfo>(*this);
...
void Tournament::sendParticipantList(bool asPart) {
    ...
    packet << players;
    for (auto&& player : participants) packet << player.id << player.name;

    if (!asPart) sendToTournamentObservers(); */
}
...
void Tournament::sendModeratorList(bool asPart) {
    ...
    for (auto&& mod_id : moderator_list) packet << mod_id;
...
void Tournament::sendGames(bool asPart) {
    ...
    for (auto&& game : bracket.games) {
        ...
        if (game.player2 != nullptr)
            ...
            packet << (uint8_t)3;
        if (game.status > 2) game.sendResults(true);
    }
...
void Tournament::sendToTournamentObservers() {
    for (auto&& client : keepUpdated) {
        ...
        if (conn.status != sf::Socket::Done) cout << "Error sending TCP packet to tournament observer " << id << endl;
    }
}
...
void Tournament::checkIfStart() {
    if (status < 2)
        if (startingTime) {
            if (time(NULL) > startingTime) startTournament();
            if (!notify)
                if (time(NULL) > startingTime - 1200) {
                    ...
                    for (auto& player : participants) cont += player.name + " ";
...
void Tournament::scoreTournament() {
    ...
    for (auto participant : participants)
        if (participant.played) jwrap.addPair(std::to_string(participant.id), participant.position);
...
void Tournament::save() {
    ...
    for (auto mod : moderator_list) file << mod << endl;
    file << players << endl;
    for (auto player : participants) file << player.id << endl << player.name << endl;
