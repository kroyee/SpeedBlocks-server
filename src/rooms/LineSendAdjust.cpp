...
void LineSendAdjust::addAdjust(Client& died, std::list<Client*>& clients, short players) {
	...
	for (auto&& client : clients) {
		if (client->roundStats.linesSent > amount) {
			second = amount;
			amount = client->roundStats.linesSent;
			if (died.id == client->id)
				leaderDied=true;
			else
				leaderDied=false;
		}
		else if (client->roundStats.linesSent > second)
			second = client->roundStats.linesSent;
	}
	if (leaderDied)
		amount = second;
