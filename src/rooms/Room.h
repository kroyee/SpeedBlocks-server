...
class Room {
   public:
    ...
    void sendPacketToPlayers(PM&);
    void sendPacketToPlayers(PM&& p) { sendPacketToPlayers(p); }
    void sendPacketToSpectators(PM&);
    void sendPacketToSpectators(PM&& p) { sendPacketToSpectators(p); }
    void sendPacketToAway(PM&);
    void sendPacketToAway(PM&& p) { sendPacketToAway(p); }
    void sendPacketToActive(PM&);
    void sendPacketToActive(PM&& p) { sendPacketToActive(p); }
...
class FFARoom : public Room {
    void scoreRound();
    void incrementGamesPlayed(Client& client) {
        ++client.stats.ffaPlayed;
        ++client.stats.totalPlayed;
    }
    void incrementGamesWon(Client& client) {
        ++client.stats.ffaWon;
        ++client.stats.totalWon;
    }
...
class HeroRoom : public Room {
    void scoreRound();
    void incrementGamesPlayed(Client& client) {
        ++client.stats.heroPlayed;
        ++client.stats.totalPlayed;
    }
    void incrementGamesWon(Client& client) {
        ++client.stats.heroWon;
        ++client.stats.totalWon;
    }
